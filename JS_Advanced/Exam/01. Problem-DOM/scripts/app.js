...
/*function solve() {
    ...
    function addBook(event) {
        ...
        const buyNewBtn = e('button', 'buy-new', `Buy it only for ${price.toFixed(2)} BGN`);
        const buyOldBtn = e('button', 'buy-old', `Buy it only for ${(price * 0.85).toFixed(2)} BGN`);
        const oldBtn = e('button', '', 'Move to old section');
        ...
        buyNewBtn.addEventListener('click', () => buyNewBook(div, price));
        buyOldBtn.addEventListener('click', () => buyOldBook(div, price));
        oldBtn.addEventListener('click', () => moveToOld(div, title, year, price));
    ...
    function buyNewBook(el, sum) {
        el.remove();
        total += sum;
        header.innerHTML = `Total Store Profit: ${total.toFixed(2)} BGN`;
    }

    function buyOldBook(el, sum) {
        el.remove();
        sum = Number((sum * 0.85).toFixed(2));
    ...
    function moveToOld(el, title, year, price) {
        ...
        const div = e('div', 'book');
        const p = e('p', '', `${title} [${year}]`);
        const buyOldBtn = e('button', 'buy-old', `Buy it only for ${price.toFixed(2)} BGN`);
